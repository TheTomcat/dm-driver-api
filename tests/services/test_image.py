...
def test_image_create_db(db: Session) -> None:
    t1 = Image(
        name="Test.png",
        path="D:\\A\\Path\\To\\Images\\Test.png",
        dimension_x=1024,
        dimension_y=1024,
        hash="1234",
    ...
    t2 = Image(
        name="Test2.png",
        path="D:\\A\\Path\\To\\Images\\Test2.png",
        dimension_x=1024,
        dimension_y=1024,
        hash="3342",
        type=ImageType.backdrop,
...
def test_image_thumbnail_validation_schema():
    ...
    ImageScale(**valid_2)
    ImageScale(**valid_3)
    ImageScale(**valid_4)  # type: ignore

    with pytest.raises(Exception):
        ImageScale(**invalid_1)


def test_image_add_remove_tag_client(
    app_client: TestClient, create_image: Image, create_tag: Tag
) -> None:
    rv = app_client.get(f"/image/{create_image.id}")
    assert rv.json()["tags"] == [], rv.json()
    rv = app_client.patch(f"/image/{create_image.id}/tag?tag_id={create_tag.id}")
    assert rv.status_code == 200, rv.text
    assert len(rv.json()["tags"]) == 1
    rv = app_client.delete(f"/image/{create_image.id}/tag?tag_id={create_tag.id}")
    assert rv.status_code == 200
    assert len(rv.json()["tags"]) == 0


def test_image_set_tags_client(
    app_client: TestClient, create_image: Image, create_tag: Tag
) -> None:
    rv = app_client.get(f"/image/{create_image.id}")
    assert rv.status_code == 200
    assert rv.json()["tags"] == []
    rv = app_client.put(f"/image/{create_image.id}/tag", json=[create_tag.id])
...
def test_image_fetch_b64_client(app_client: TestClient, create_real_images: list[Image]) -> None:
